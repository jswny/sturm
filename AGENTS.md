...
- For Code Mode failures, use AI Gateway logs to inspect the outer model turn and `codemode` tool call, then use the Access-protected `POST /api/admin/codemode/inspect` endpoint with the guild/channel surface and `correlationId` when available to inspect the inner Code Mode runtime audit trail. Compare timestamps, `correlationId`, `executionId`, status, code preview, connector list, tool-call summaries, and errors; remember the endpoint returns bounded previews, not full unbounded payloads.
...
- Code Mode `describe` renders some Zod/JSON Schema validation details as plain TypeScript types, so model-facing tool descriptions must manually state argument-selection constraints the model needs to choose valid values, such as numeric ranges, length limits, and pattern requirements. Keep the Zod schema as the enforcement source, and derive description text from the same constants when practical so the model-facing docs and runtime validation do not drift.
- Do not add AI SDK `toModelOutput` formatters to normal chat tools unless a real non-Code-Mode tool-calling path is introduced and verified. Sturm's normal `/c` path runs chat tools behind Code Mode, where the sandbox discovers tools through descriptions and schemas and generated code receives the normal tool return value; `toModelOutput` is not expected to reach the outer model. Put static behavior rules, final-response rules, argument-selection guidance, and warnings in the tool description/schema or broader prompt layer. If dynamic details from an inner tool need to affect the final response through Code Mode, the generated code must include the inner tool's return value, or a concise structured summary of it, in the Code Mode function's final returned value. Do not rely on `console.log` for model-visible structured data; Code Mode captures logs as text, and object logs may collapse to `[object Object]`.
...
- Tools should return clear structured model-facing results that state success or failure and the concrete action taken. Keep normal return values useful to Code Mode generated code and durable chat history as a log of write tools and other tool activity.
