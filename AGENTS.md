...
- `/memory` is the guild-scoped admin surface for guild memory. Keep `view`, ID-based `delete`, and `reset` admin-only and ephemeral; `/memory reset` clears guild-scoped memory and advances its reset epoch.
