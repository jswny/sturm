...
- Discord interactions should return the deferred Discord response immediately, then enqueue the per-conversation Agent work through `ctx.waitUntil()`. Do not run model/tool work from the `/discord` route; the route may only validate, classify, acknowledge, and hand off to the durable Agent queue.
