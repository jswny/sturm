...
- For Code Mode tools, rely on the Zod input schema as the primary argument contract. Do not duplicate the same scalar validation in runtime code unless there is a real non-model caller path or another boundary that can bypass the schema.
