...
- Image generation is a chat tool backed by Workers AI in `src/images.ts` and stores artifacts in the `ARTIFACTS_BUCKET` R2 binding (`sturm-artifacts`) under `images/generated/`. Treat generated images as response artifacts: send them as Discord attachments or debug response data, but persist only R2 keys and metadata in durable delivery/session state. Never store raw base64/image bytes in Session history, Think submissions, or delivery records.
