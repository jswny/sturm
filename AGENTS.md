...
- Use the Cloudflare MCP/OpenAPI before guessing at platform behavior. For AI Gateway issues, inspect account `40444d2f11999fee857be59abe745754`, gateway `default`, and prefer logs filtered by Sturm metadata (`app: "sturm"`, `interactionId` when available, plus `flow: "reply"`, `"compaction"`, or `"memory-reflection"`). Check the log row first for model, provider, status, duration, token counts, path, and timestamp, then inspect `/logs/{id}/request` and `/logs/{id}/response` when payload retention is enabled.
...
- Treat `interactionId` as the primary per-turn correlation ID. When a Discord/debug turn fans out into Think submissions, Discord delivery records, Code Mode executions, AI Gateway metadata, artifacts, memory reflection, or app logs, preserve or reference the originating `interactionId` wherever the boundary supports it. Do not use `interactionId` as authorization, conversation identity, or long-term domain identity; those remain explicit guild/channel/user IDs and conversation names.
