...
- Discord `beforeTurn()` should compose the final system prompt from `createSystemPrompt()`, a fresh `session.refreshSystemPrompt()` result, and Discord runtime constraints. Refreshing every turn favors guild-memory correctness over prompt snapshot caching; only optimize this with explicit context invalidation after memory writes.
