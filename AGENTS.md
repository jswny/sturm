...
- Current-turn Discord image artifacts should be summarized into concise `visualSummary` metadata for future text-only turns. Generate summaries as a per-turn side effect that runs in parallel with the main reply but blocks final delivery until complete or timed out using the same reply timeout. Keep summary generation separate from the user-visible assistant response, use the configured summary model/settings, and prefer a plain image-summary prompt with structured output over brittle content-specific cleanup. Do not fail the main turn if summary generation cannot complete.
