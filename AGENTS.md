...
- Persist runtime-owned provenance on every guild memory record as `discord_turn` or `ambient_channel`, alongside its source correlation ID. Do not expose this source enum in model-facing guild memory or reflection context; show it only on operational surfaces such as `/memory view`.
