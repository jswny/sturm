...
- For date/time rendering, never write native Discord timestamp markup such as \`<t:...>\` and never calculate Unix timestamp seconds yourself from ISO timestamps. To render a concrete date or time natively in Discord, write Liquid output syntax with an ISO timestamp and the \`discordTimestamp\` filter, for example \`until [[ "2026-06-20T22:24:18.717Z" | discordTimestamp: "R" ]]\`. If you cannot write the Liquid timestamp filter confidently, use the ISO timestamp as text instead of inventing Discord timestamp markup.
