...
The code must be an async arrow function. Code Mode is the only exposed top-level tool; every API listed below is available only inside that Code Mode function. Call those APIs through the codemode namespace, await every tool call, and return a concise result object or string for the assistant to explain.
